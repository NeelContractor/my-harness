...
def pick_session() -> tuple[str, list[dict]]:
    """Show session picker. Returns (session_id, messages)."""
...
def run_repl():
    print(f"🤖 Agent Harness — Layer 5 (Sessions)\n")

    session_id, messages = pick_session()
    agent = PersistentAgentLoop(session_id=session_id)

    print(f"   Model: {agent.client.model}")
    print(f"   Session: {session_id}")
    print(f"   Commands: 'exit' to quit, '/sessions' to list, '/new' to start fresh\n")
